...
// --- CSV parsing (quoted fields supported) ---
  ...
  function parseBackupText(text) {
    ...
    // Preferred format: full JSON object from exportAllStorage()
    try {
      const obj = JSON.parse(raw);
      if (obj && typeof obj === 'object' && !Array.isArray(obj)) return obj;
    } catch (e) {}

    // Fallback: devtools copy/export style with one key/value per line separated by tab or spaces.
...
/).filter(Boolean);
    ...
    if (!Object.keys(obj).length) throw new Error('Could not read backup data.');
    return obj;
  }

  function importAllStorageObject(obj) {
  ...
  function wireBackupUI(doc) {
    const root = doc || document;
    const exportBtn = root.getElementById('bt-export-backup');
    const importBtn = root.getElementById('bt-import-backup');
    const importFile = root.getElementById('bt-import-file');
    const status = root.getElementById('bt-backup-status');
    if (!exportBtn || !importBtn || !importFile || !status) return;

    exportBtn.addEventListener('click', () => {
      try {
        const data = exportAllStorage();
        const stamp = new Date().toISOString().replace(/[:.]/g, '-');
        downloadTextFile(`bowling-tool-full-backup-${stamp}.json`, JSON.stringify(data, null, 2));
